...
import { calculateAreaRiskScore, dhakaAreas } from '../utils/crimeData';
...
export const useLocationMonitor = (crimes: any[]) => {
...
const findNearestArea = (lat: number, lng: number): { name: string; lat: number; lng: number } | null => {
  let nearestArea: { name: string; lat: number; lng: number } | null = null;
  let minDistance = Infinity;

  dhakaAreas.forEach((area: any) => {
