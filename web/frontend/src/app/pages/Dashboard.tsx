...
import { generateCrimeData } from '../utils/crimeData';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (startDate) {
      filtered = filtered.filter(crime => new Date(crime.date) >= new Date(startDate));
    }

    if (endDate) {
      filtered = filtered.filter(crime => new Date(crime.date) <= new Date(endDate));
    }
